...
def _make_split_loader(n=4, seq_len=32, bio_seq_len=16):
    input_ids = torch.randint(0, 1000, (n, seq_len))
    attention_mask = torch.ones(n, seq_len, dtype=torch.long)
    bio_input_ids = input_ids[:, :bio_seq_len].clone()
    bio_attention_mask = torch.ones(n, bio_seq_len, dtype=torch.long)
    bio_labels = torch.zeros(n, bio_seq_len, dtype=torch.long)
    bio_labels[:, 1] = 1  # B-ASP
    bio_labels[:, 2] = 2  # I-ASP
    bio_labels[:, 0] = -100  # [CLS]
    sentiment_label = torch.tensor([0, 1, 2, 0][:n], dtype=torch.long)
    ...
    ds = TensorDataset(input_ids, attention_mask, bio_input_ids,
                        bio_attention_mask, bio_labels, sentiment_label, crf_mask)
    ...
    class _Wrapper:
        ...
        def __iter__(self):
            loader = DataLoader(self._ds, batch_size=self._bs)
            for ids, mask, bio_ids, bio_mask, bio, sent, crf_m in loader:
                yield {
                    "input_ids": ids,
                    "attention_mask": mask,
                    "bio_input_ids": bio_ids,
                    "bio_attention_mask": bio_mask,
                    "bio_labels": bio,
                    "sentiment_label": sent,
                    "crf_mask": crf_m,
                }
...
def test_split_evaluate_returns_expected_keys():
    torch.manual_seed(0)
    model = RetrievalABSA()
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)
    trainer = ABSATrainer(model, optimizer, scheduler=None,
                          device="cpu", log_path="")
    loader = _make_split_loader()
    result = trainer.evaluate(loader)
    expected_keys = {"loss", "bio_token_f1", "span_f1",
                     "sentiment_acc", "sentiment_macro_f1", "joint_f1"}
    assert expected_keys.issubset(set(result.keys()))
...
def test_split_train_returns_history():
    torch.manual_seed(0)
    model = RetrievalABSA()
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)
    trainer = ABSATrainer(model, optimizer, scheduler=None,
